...
/**
 * How many consecutive days a meal needs to be planned on. A recipe yields
 * `baseServings` portions as written; when the household eats fewer people
 * per sitting than that, the leftovers get spread over the following day(s)
 * instead of being scaled down. Capped at 3 days total so leftovers are
 * always eaten within a day or two, never later.
 */
const requiredDays = (meal: Meal, servings: number): number =>
  Math.min(
    3,
    Math.max(1, Math.ceil(meal.baseServings / Math.max(1, servings))),
  );

const scaleFactor = (meal: Meal, servings: number): number =>
  (servings * requiredDays(meal, servings)) / meal.baseServings;

const scaleAmount = (amount: number, unit: string, factor: number): string => {
...
const mealPrice = (meal: Meal, servings: number): number =>
  meal.price * scaleFactor(meal, servings);

const dayFull = (key: string | null): string =>
  DAYS.find((day) => day.key === key)?.full ?? "";

const dayRangeLabel = (days: string[]): string => {
  if (!days.length) {
    return "Vrij";
  }

  if (days.length === 1) {
    return dayFull(days[0]);
  }

  const extra = days.length - 1;

  return `${dayFull(days[0])} + ${extra} dag${extra > 1 ? "en" : ""}`;
};
...
/**
 * Consecutive day keys starting at `startKey`, or null if the span would run
 * past the end of the week.
 */
const consecutiveDaysFrom = (
  startKey: string,
  span: number,
): string[] | null => {
  const startIndex = dayIndex(startKey);

  if (startIndex === -1 || startIndex + span > DAYS.length) {
    return null;
  }

  return DAYS.slice(startIndex, startIndex + span).map((day) => day.key);
};

const daysOccupiedByOthers = (excludeId: number): Set<string> => {
  ...
  for (const key of Object.keys(selections.value)) {
    ...
    for (const day of selections.value[id].days) {
      occupied.add(day);
    }
  }
...
const openDetail = (id: number): void => {
  ...
  detailDay.value =
    selection && selection.days.length ? selection.days[0] : null;
};
...
const detailRequiredDays = computed(() =>
  detailMeal.value ? requiredDays(detailMeal.value, detailServings.value) : 1,
);

const detailDays = computed<string[]>(() => {
  const meal = detailMeal.value;

  if (!meal || !detailDay.value) {
    return [];
  }
  ...
  return (
    consecutiveDaysFrom(
      detailDay.value,
      requiredDays(meal, detailServings.value),
    ) ?? []
  );
});

const detailDayFits = computed(
  () => detailDay.value === null || detailDays.value.length > 0,
);

const chooseDetailDay = (key: string): void => {
  const meal = detailMeal.value;
  ...
  if (detailDay.value === key) {
    detailDay.value = null;

    return;
  }

  const span = requiredDays(meal, detailServings.value);
  const days = consecutiveDaysFrom(key, span);

  if (!days) {
    showToast("Past niet meer in de week vanaf deze dag");

    return;
  }

  const occupied = daysOccupiedByOthers(meal.id);

  if (days.some((day) => occupied.has(day))) {
    showToast("Een van die dagen is al bezet");
...
const clearDaysOnOtherMeals = (
  ...
  for (const key of Object.keys(next)) {
    ...
    if (id !== keepId && next[id].days.some((day) => days.includes(day))) {
      next[id] = {
        ...next[id],
        days: next[id].days.filter((day) => !days.includes(day)),
      };
    }
  }
};
...
const saveDetail = (): void => {
  ...
  if (detailDay.value && detailDays.value.length === 0) {
    showToast("Kies een geldige dag");
  ...
  if (detailDays.value.length) {
    clearDaysOnOtherMeals(next, id, detailDays.value);
  }

  next[id] = { servings: detailServings.value, days: detailDays.value };
...
const changeServings = (id: number, delta: number): void => {
  ...
  const servings = Math.min(8, Math.max(1, current.servings + delta));
  let days = current.days;

  if (days.length) {
    const span = requiredDays(meal, servings);
    const candidate = consecutiveDaysFrom(days[0], span);
    const occupied = daysOccupiedByOthers(id);
    const stillFits =
      !!candidate && !candidate.some((day) => occupied.has(day));
    days = stillFits ? (candidate as string[]) : [];

    if (!stillFits) {
      showToast("Dag opnieuw plannen door aangepaste porties");
    }
  }

  selections.value = { ...selections.value, [id]: { servings, days } };
};

const setEntryDay = (id: number, key: string): void => {
  const current = selections.value[id];
  const meal = mealById(id);
  ...
  if (current.days[0] === key) {
    selections.value = { ...selections.value, [id]: { ...current, days: [] } };

    return;
  }

  const span = requiredDays(meal, current.servings);
  const days = consecutiveDaysFrom(key, span);
  ...
  const next = { ...selections.value };
  clearDaysOnOtherMeals(next, id, days);
  next[id] = { ...current, days };
  selections.value = next;
};
...
const selectedEntries = computed(() =>
  Object.keys(selections.value)
    .map((key) => {
      ...
      return meal
        ? { id, meal, servings: selection.servings, days: selection.days }
        : null;
    })
...
const calendarDays = computed(() =>
  DAYS.map((day) => {
    const entry = selectedEntries.value.find((entry) =>
    ...
    return {
      ...day,
      entry,
      isLeftover: entry ? entry.days[0] !== day.key : false,
    };
  }),
);
...
const totalPortions = computed(() =>
  selectedEntries.value.reduce(
    (sum, entry) => sum + entry.servings * Math.max(1, entry.days.length),
    0,
  ),
);
...
const successEntries = computed(() =>
  [...selectedEntries.value].sort((a, b) => {
    const orderA = a.days.length ? dayIndex(a.days[0]) : 99;
    const orderB = b.days.length ? dayIndex(b.days[0]) : 99;
      ...
      <header class="wm-header">
        <div class="wm-header-row">
          ...
          <button class="wm-surprise" @click="surprise">
            ...
            >
              <path
                d="M12 3l1.9 4.3L18 9l-4.1 1.7L12 15l-1.9-4.3L6 9l4.1-1.7z"
              />
              <path
                d="M19 14l.8 1.8L21.5 17l-1.7.7L19 19l-.8-1.3-1.7-.7 1.7-1.2z"
              />
            </svg>
            Verras me
          </button>
            ...
            <div class="wm-card-body">
              ...
              <div v-if="selections[meal.id]" class="wm-card-selinfo">
                {{ selections[meal.id].servings }} pers.<template
                  v-if="selections[meal.id].days.length"
                >
                  &middot;
                  {{ dayRangeLabel(selections[meal.id].days) }}</template
                >
              </div>
            </div>
          </div>
          ...
          <div class="wm-calendar">
            <div v-for="day in calendarDays" :key="day.key" class="wm-cal-day">
              ...
              >
                ...
                <div v-if="day.isLeftover" class="wm-cal-leftover">Restjes</div>
              </div>
          ...
          <div v-if="selectedEntries.length" class="wm-plan-list">
            ...
            >
              ...
              <div class="wm-plan-info">
                ...
                <div
                  v-if="requiredDays(entry.meal, entry.servings) > 1"
                  class="wm-plan-days-hint"
                >
                  Dekt {{ requiredDays(entry.meal, entry.servings) }} dagen
                </div>
                <div class="wm-plan-days">
                  <button
                    ...
                    :class="{
                      'wm-day-pill-active': entry.days.includes(day.key),
                      'wm-day-pill-leftover':
                        entry.days.includes(day.key) &&
                        entry.days[0] !== day.key,
                    }"
                    @click="setEntryDay(entry.id, day.key)"
                  >
                    {{ day.short }}
                  </button>
                </div>
        ...
        <aside class="wm-summary">
          ...
          >
            ...
            >
              ...
              <path
                d="M2 3h2.2l2.3 12.4a2 2 0 0 0 2 1.6h8.7a2 2 0 0 0 2-1.5L23 7H5.2"
              />
            </svg>
    ...
    <section v-else class="wm-success">
      ...
      <div class="wm-success-card">
        ...
        <div class="wm-success-total">
          ...
          <span class="wm-success-total-val">{{
            formatPrice(totalPrice)
          }}</span>
        </div>
        ...
        <div class="wm-detail-content">
          <div class="wm-scroll wm-detail-scroll">
            ...
            <div class="wm-detail-days-block">
              ...
              <div v-if="detailRequiredDays > 1" class="wm-detail-days-hint">
                Dit gerecht is voor {{ detailMeal?.baseServings }} personen
                &middot; bij {{ detailServings }} personen dekt het
                {{ detailRequiredDays }}
                dagen. De dag erna (of de dag daarna) wordt automatisch
                meegepland voor de restjes.
              </div>
              <div
                v-if="!detailDayFits"
                class="wm-detail-days-hint wm-detail-days-warning"
              >
                Past niet meer vanaf deze dag, kies een eerdere dag.
              </div>
              <div class="wm-detail-days">
                <button
                  ...
                  :class="{
                    'wm-day-pill-active': detailDays.includes(day.key),
                    'wm-day-pill-leftover':
                      detailDays.includes(day.key) && detailDays[0] !== day.key,
                  }"
                  @click="chooseDetailDay(day.key)"
                >
                  {{ day.short }}
                </button>
              </div>
                  ...
                  <span class="wm-ingredient-amount">
                    {{
                      scaleAmount(
                        ...
                        ingredient.unit,
                        detailMeal
                          ? scaleFactor(detailMeal, detailServings)
                          : 1,
                      )
                    }}
                  </span>
